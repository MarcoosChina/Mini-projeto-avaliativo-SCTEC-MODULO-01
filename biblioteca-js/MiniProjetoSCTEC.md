...
## SCRIPT DE LEITURA PARA VÍDEO - EXPLICAÇÃO DOS CÓDIGOS

### 1. realizarEmprestimo.js

"Vou explicar o código do arquivo realizarEmprestimo.js, que é responsável por processar o empréstimo de livros no sistema da biblioteca.

O código começa importando o array de livros do arquivo dadosLivros.js usando o require. Isso é necessário para acessar os dados dos livros cadastrados no sistema.

Em seguida, temos a função principal chamada realizarEmprestimo, que recebe como parâmetro o título do livro que será emprestado.

Dentro da função, a primeira ação é buscar o livro no array usando o método find. O find procura um livro cujo título corresponda ao título informado, convertendo ambos para minúsculas para garantir que a busca não seja sensível a maiúsculas e minúsculas.

Depois de encontrar o livro, o código verifica duas condições usando o operador lógico AND: se o livro foi encontrado E se a propriedade disponivel é true. Isso significa que o livro existe e está disponível para empréstimo.

Se ambas as condições forem verdadeiras, o código altera a propriedade disponivel do livro para false, indicando que ele agora está emprestado. Em seguida, exibe a mensagem 'Livro emprestado com sucesso!' no console.

Caso contrário, se o livro não existir ou não estiver disponível, o código exibe a mensagem 'Livro indisponível!' no console.

Por fim, a função é exportada usando module.exports para que possa ser utilizada em outros arquivos do projeto, especialmente no index.js onde o menu principal chama esta função."

### 2. realizarDevolucao.js

"Agora vou explicar o código do arquivo realizarDevolucao.js, que é responsável por processar a devolução de livros no sistema.

Similar ao arquivo anterior, o código começa importando o array de livros do arquivo dadosLivros.js usando o require.

A função principal é chamada realizarDevolucao e também recebe como parâmetro o título do livro que será devolvido.

Dentro da função, a primeira ação é buscar o livro no array usando o método find, da mesma forma que no código de empréstimo, convertendo os títulos para minúsculos para garantir a correspondência independente de maiúsculas e minúsculas.

A diferença aqui é que a verificação é mais simples: o código apenas verifica se o livro foi encontrado, sem verificar a disponibilidade. Isso porque, na lógica de devolução, assumimos que qualquer livro encontrado pode ser devolvido, independentemente do seu estado atual.

Se o livro for encontrado, o código altera a propriedade disponivel do livro para true, indicando que ele agora está disponível novamente para empréstimo. Em seguida, exibe a mensagem 'Livro devolvido com sucesso!' no console.

Caso o livro não seja encontrado, o código exibe a mensagem 'Livro não encontrado!' no console.

Por fim, a função é exportada usando module.exports para ser utilizada em outros arquivos do projeto."

### 3. exibirEstatisticas.js

"Por fim, vou explicar o código do arquivo exibirEstatisticas.js, que é responsável por gerar e exibir estatísticas detalhadas sobre o acervo da biblioteca.

O código também começa importando o array de livros do arquivo dadosLivros.js.

A função principal é chamada exibirEstatisticas e não recebe parâmetros, pois ela trabalha com todos os livros do array.

O primeiro cálculo é obter o total de livros cadastrados usando a propriedade length do array.

Em seguida, o código calcula a quantidade de livros disponíveis usando o método filter. O filter cria um novo array contendo apenas os livros onde a propriedade disponivel é true, e então obtemos o tamanho desse array com length.

De forma similar, calculamos a quantidade de livros em empréstimo usando filter, mas desta vez buscando livros onde a propriedade disponivel é false, ou seja, usando o operador de negação.

A quantidade de livros que deram entrada no catálogo é igual ao total de livros, pois no sistema atual todos os livros cadastrados estão no catálogo.

Depois, o código calcula as porcentagens correspondentes. Para cada categoria, dividimos a quantidade pelo total de livros, multiplicamos por 100 e usamos Math.round para obter um valor inteiro, eliminando as casas decimais.

O código também calcula o total de páginas de todos os livros usando o método reduce. O reduce percorre o array acumulando a soma das páginas de cada livro, começando de zero.

A média de páginas por livro é calculada dividindo o total de páginas pelo número de livros e usando Math.round para arredondar para um valor inteiro.

Por fim, todas essas estatísticas são exibidas no console de forma organizada, mostrando tanto as quantidades quanto as porcentagens correspondentes. Cada estatística é apresentada com uma descrição clara, incluindo o total de livros cadastrados, livros no catálogo, livros disponíveis, livros em empréstimo, total de páginas e média de páginas por livro.

A função é exportada usando module.exports para ser utilizada no menu principal do sistema."
